...
namespace SharpMikXNA.Drivers
{
	public class XNADriver : VirtualDriver1
	{
		...
		{
			NextDriver = null;
			Name = "XNA Dynamic Sound Effect Driver";
			Version = "XNA Driver v1.0";
			HardVoiceLimit = 0;
			SoftVoiceLimit = 255;
			AutoUpdating = true;
			m_Playing = false;
		...
		public override bool Init()
		{
			...
			m_SoundInstance = new DynamicSoundEffectInstance(ModDriver.MixFrequency, (ModDriver.Mode & Constants.DMODE_STEREO) == Constants.DMODE_STEREO ? AudioChannels.Stereo : AudioChannels.Mono);
		...
		void m_SoundInstance_BufferNeeded(object sender, EventArgs e)
		{
			if (m_Playing)
			{
				...
				{
					var done = VC_WriteBytes(m_SignedAudiobuffer, (uint)BUFFERSIZE);
